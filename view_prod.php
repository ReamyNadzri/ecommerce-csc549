...
<body>
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-6 d-flex flex-column gap-4">
                        ...
                        <div class="d-flex gap-3">
                            <button
                                onclick="addToCart()"
                                class="btn btn-custom-solid w-100 py-2 fw-medium d-flex align-items-center justify-content-center gap-2">
    ...
    <script>
        ...
        function addToCart() {
            // 1. Get the product details from PHP variables embedded in the page
            const productId = <?php echo json_encode($product['ProductID']); ?>;
            const brandName = <?php echo json_encode($product['BrandName']); ?>;
            const flavour = <?php echo json_encode($product['Flavour']); ?>;
            const price = <?php echo json_encode($product['PriceMYR']); ?>;
            const image = <?php echo json_encode($product['ImagePath']); ?>;
            const description = <?php echo json_encode($product['Description']); ?>;

            // 2. Get the selected quantity from the input field
            ...
            formData.append('quantity', quantity); // Send the selected quantity
            formData.append('ajax', '1');

            // 4. Send the data to index.php
            fetch('index.php', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showNotification('Item(s) added to cart!', 'success');
                    } else {
        ...
        // Helper function to update the cart number in the header
        function updateCartBadge(count) {
            const badge = document.getElementById('cart-badge'); // Make sure your header has an element with id="cart-badge"
            if (badge) {
        ...
        // Helper function to show a pop-up notification
        function showNotification(message, type) {
            const notification = document.createElement('div');
            notification.className = `alert alert-${type === 'success' ? 'success' : 'danger'} position-fixed top-0 end-0 m-4 shadow-lg`;
